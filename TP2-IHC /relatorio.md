...
Além disso, podemos observar que a árvore de decisão do algoritmo J48 presente no 5.3 contém apenas uma condição de um único atributo, sendo este a raiz da árvore. Caso o atributo "desvio_sequencia" seja maior que 0, então o aluno está com dificuldade. No entanto, se "desvio_sequencia" seja menor ou igual a 0, então o aluno está progredindo. A condição que o algoritmo encontrou para descriminar perfeitamente os dois grupos da classe alvo é a mesma condição que foi definida para classificar instâncias como "com_dificuldade" e "progredindo". Também pdemos verificar que as condições originais para a classe alvo presente na seção dois possuem mais dois atributos para cada: "revisitas_painel_principal" e "progresso_no_modulo" para alunos "com dificuldade", e "tempo_medio_recurso" e "progresso_no_modulo" para alunos "progredindo". Mesmo assim, o algoritmo J48 utilizou apenas o terceiro atributo, e conseguiu discriminar completamente os dois grupos.
